...
!    =================================================================
     subroutine apexd_coil(ci1,rs1,rs2,zs1,zs2, xm,zm, Bxm,Bzm,Bs,rAm)
!    =================================================================
     implicit none
...
!f2py intent(out) :: Bxm,Bzm,Bs,rAm
     double precision :: Bx,Bz,Bxt,Byt,Bzt,rAmt
     integer :: nxmax,nzmax,nzmid,i,j,nrc1,nzc1,ic,jc,nx,nz
     double precision,dimension(0:100005) :: elik,elie
     double precision :: x,z,dx,dz,xi,at,rsd1,zsd1,zds1,rds1,csq1,xc1
     double precision :: xmin,xmax,zmin,zmax
!    ----------------------------------

!    Computation parameters    
!    Calculation region grid number and area. nx and nz must be even.
     nx = 50; nz = 50;
     xmin = -200.0d-3; xmax = 200.0d-3;
     zmin = -200.0d-3; zmax = 200.0d-3; 

!    Definition of coil size and geometry
...
!    Obtain elliptic functions
     ...
     IF ( file_exists ) then
         OPEN (50,File="el2.dat")
          DO I=0,99999
               READ(50,*) xi,elik(i),elie(i)
          end do
          CLOSE(50)
     else
        call efile(elik,elie) ! calculated K and E are saved as arrays
         OPEN (2,File="el2.dat")
         DO i=0,99999
            write(2,703) I/100000.0d0,elik(i),elie(i)
         end do
         CLOSE(2)
     endif 
...
!          ### Superpose coil currents for B
           ...
           do ic=0,nzc1
               ...
               do jc=0,nrc1
                  ...
                  call bloop(elik,elie,rds1,dabs(x-xc1),z-zds1,ci1*csq1,Bx,Bz)
...
!          ### Superpose coil currents for rAtheta
           rAmt=0.0d0
           ...
           do ic=0,nzc1
              ...
              do jc=0,nrc1
                 rds1 = rs1 + rsd1*dble(jc)
                 call rAtheta(elik,elie,rds1,dabs(x-xc1),z-zds1,ci1*csq1,at)
...
!    Useful format definition
     703  format(1p3d23.15)

    end subroutine apexd_coil
...
!     Linear interpolation of FK and FE end
...
!     =================================================================
      SUBROUTINE rAtheta(elik,elie,rc,x,z,ci,at)
...
!                                                   ===================
      implicit none
      double precision, intent(in), dimension(0:100005) :: elik,elie
...
!    Linear interpolation of FK and FE start
      i2=i+1;
      fk1=elik(i); fe1=elie(i);
      fk2=elik(i2); fe2=elie(i2);
!      write(6,'(d13.5,i7,1p3d17.9)') zzk,i,fk1,fk2,fk1+(zzk-dble(i)/1.0d5)*(fk2-fk1)
      fk=fk1+(zzk*1.0d5-dble(i))*(fk2-fk1);
      fe=fe1+(zzk*1.0d5-dble(i))*(fe2-fe1);
...
!      FE=elie2(I)
 ...
 20   AT=0.0d0
      ...
      END SUBROUTINE rAtheta



!     =================================================================
      SUBROUTINE EFILE(elik,elie)
!     complete elliptic fuctions
!     computation speed/accuracy may be adjusted by setting nmax x2
!     =================================================================
!                                                    elliptic functions
!                                                    ==================
      implicit none
...
!     #######################################
!     Complete elliptic integral functions
!     - - - - - - - - - - - - - - - - - - - -       
      double precision function FEF1(k)  ! Complete elliptic integral of 1st kind 
         implicit none
      ...
      double precision function FEF2(k)  ! Complete elliptic integral of 2nd kind 
         implicit none
         double precision, intent(in) :: k  !
         double precision :: pi, m, dt, t, tmin, tmax
         integer :: i
