!C    =================================================================
!C    calculate magnetic field by ring and line current
!C      checked with gcc 7.1.0, MinGW-W64, 28.11.2017
!C      line loop current, coil volume integral, line (element) current
!
!C    How it works:
!C      According to set values (coil position, size, etc.), magnetic field
!C      and flux function are calculated using complete elliptic function.
!C      can be used for calculations of B field inside SC coil.
!
!C    How to use:
!C      In "set parameters", decide coil parameters.
!C      Only for the first run, set itf=1 so that elliptic function data
!C      el2.dat are generated. As far as this file exits, you may set itf=0.
!C      Then elliptic function data are loaded from this file, which reduces
!C      calculation time.
!C      Data files are saved as b.txt and bl.txt. See comments.
!C
!C    =================================================================
